...
function Button({text, small, big, icon, full, className, ...buttonProps}) {
    let medium
    if (!small && !big) {
        medium = true
    }
    ...
    return (
        <button
            className={clsx(
                ...
                ['text-white disabled:text-zinc-300', {'text-sm': small, 'text-base': medium}],
        ...
        >
            {icon ? (
                <div className={clsx('h-full flex items-center', {'mr-1': small, 'mr-2': medium, 'mr-3': big})}>
                    <Icon
